class Unmined {
    ...
    map(mapId, options, regions) {
        ...
        if (options.markers) {
            ...
            const markerSwitch = document.querySelector(".switch")

            markerSwitch.addEventListener("change", (e) => {
                if (e.target.checked) {
                    map.addLayer(markersLayer);
                } else {
                    map.removeLayer(markersLayer)
                }
                console.log(unmined.openlayersMap.removeLayer)
            })
        }
            ...
            if (item.text) {
                style.setText(new ol.style.Text({
                    ...
                    backgroundFill: new ol.style.Fill({
                        color: item.textBackgroundColor ?? "rgba(0,0,0,0.3)"
                    }),
